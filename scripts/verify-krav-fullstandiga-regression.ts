...
// incidentfall: krav_fullstandiga: true kan BARA sättas av ett
// researchpass som faktiskt bedömt att kravlistan är komplett — till
// skillnad från belopp_status/deadline_status/krav_status (som är
// mekaniska, härledda ur om fältet har ett värde) är krav_fullstandiga
// en bedömning som inte går att räkna om. Just därför är den UNDANTAGEN
// från omrakna-datatillstand.ts (se den filens huvudkommentar) — men
...
// råkar ta bort raden. Detta skript är det oberoende skyddsnätet: fäller
// om en bidragspost som HADE krav_fullstandiga: true tappar den, om inte
// ändringen uttryckligt markeras i commit-meddelandet.
//
...
const MARKER_PATH = '.git/KRAV_FULLSTANDIGA_REGRESSION_PENDING.json';
...
function kravFullstandigaIds(yamlText: string): Set<string> | null {
  try {
    ...
    return new Set(doc.bidrag.filter((b) => b.krav_fullstandiga === true).map((b) => b.id));
  } catch {
...
if (motArg) {
  ...
  console.log('Regressionsskydd: krav_fullstandiga: true tappat för:');
  ...
  console.error('BLOCKERAD — krav_fullstandiga: true tappat utan att "minskning" nämns i commit-meddelandet:');
  ...
  console.error('BLOCKERAD (CI) — krav_fullstandiga: true tappat utan att "minskning" nämns i någon commit i intervallet:');
  for (const r of regressioner) console.error(`  ${r.kommun} — ${r.bidragId}`);
  process.exit(1);
} else {
